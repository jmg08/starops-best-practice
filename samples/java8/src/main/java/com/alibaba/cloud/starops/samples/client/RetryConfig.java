...
public class RetryConfig {
     ...
     * - STAROPS_MAX_RETRIES：最大重试次数
     * - STAROPS_IDLE_TIMEOUT：空闲超时（秒），内部统一转换为毫秒
     */
    public static RetryConfig loadFromEnv() {
        return loadFromEnv(null);
    }

    /**
     * 从环境变量和 .env 加载重试配置
     * Load retry config from environment variables and .env
     */
    public static RetryConfig loadFromEnv(Dotenv dotenv) {
        RetryConfig cfg = defaultConfig();

        String maxRetries = getConfigValue(dotenv, "STAROPS_MAX_RETRIES");
        ...
        // 环境变量按秒配置，内部统一转换为毫秒
        String idleTimeout = getConfigValue(dotenv, "STAROPS_IDLE_TIMEOUT");
